...
fn count_in_binary(c: &mut chooser::Chooser) {
    let v = vec![0, 1];
    println!("{0} {1} {2}", c.choose(&v), c.choose(&v), c.choose(&v));
}

fn magic_square(c: &mut chooser::Chooser) {
...
fn main() {
    chooser::run_choices(count_in_binary);
    println!();
    chooser::run_choices(magic_square);

    chooser::run_choices(|c| sudoku::solve(
        c,
        [
            [0, 3, 0, 6, 0, 0, 0, 8, 0],
            [0, 0, 9, 8, 0, 1, 7, 0, 2],
